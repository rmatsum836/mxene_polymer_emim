...
def compute_angles(top_file, trj_file):
    ...
    for n_frame, frame in enumerate(universe.trajectory[::1]):
        for n_ring, ring_group in enumerate(ring_groups):
            ...
            #if ring_group.center_of_mass()[2] > 28.47 + 5.0 or ring_group.center_of_mass()[2] < 28.47:
            if ring_group.center_of_mass()[2] > 6.82 + 5.0:
                continue 
            # Only consider ions in the pore
            #if ring_group.center_of_mass()[1] < 1.5 or ring_group.center_of_mass()[1] > 5.5:
            #    continue
            print(n_ring)
            xyz = ring_group.positions 
            # vector of ring com x,y, and normal of ring
            tmp_A = []
            # ring com z coordinate
            tmp_b = []
            ...
            ring_angles[n_frame, n_ring] = angle_between([fit[0], fit[1], -1], [0, 0, 1]) * (180 / np.pi)
